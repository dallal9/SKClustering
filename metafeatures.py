...
def meta_features_model_based(X, y):
    metafeatures = dict()
...
def all_metafeatures(data, num_cols, metafeatures1):
    ...
    metafeatures3 = new_metafeatures(data)
    metafeatures4 = meta_features_model_based(X, y)
    metafeatures = dict_merge(metafeatures1, metafeatures2, 
                                metafeatures3, metafeatures4)
    return metafeatures

def all_metafeatures_db(data):
    metafeatures2 = basic_statistics(data)
    metafeatures3 = interval_percentages(data)
    metafeatures4 = zscore_interval(data)
    metafeatures = dict_merge(metafeatures2, metafeatures3, metafeatures4)
    return metafeatures
    

def extract_metafeatures(file, meta_type):
    ...
    if missing_val != 0:
        ...
        for index, num_imput_type in enumerate(imputation_types):          
            ...
            if meta_type == "attribute":
                ...
                imputed_data[num_cols] = numeric_impute(data, num_cols, num_imput_type)
                metafeatures = all_metafeatures_db(imputed_data)
        ...
        if meta_type == "attribute":
            ...
            metafeatures = all_metafeatures_db(distance_transform(data))
            results = pd.DataFrame([metafeatures])
...
def extract_for_all(path, meta_type):
    ...
    for idx, file in enumerate(allFiles):
        ...
        results = pd.concat([results, extract_metafeatures(file, meta_type)], axis=0)
